...
public class DateUtil {
	...
	public static final String PATTERN_UTC_DATETIME = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'";
	public static final String PATTERN_CUSTOM_DATE = "yyyy-MM-dd[['T'HH][:mm][:ss]]";
	public static final String PATTERN_CUSTOM_DATE_SLASH = "yyyy/MM/dd[['T'HH][:mm][:ss]]";
	...
	/**
	 * 根据传入的时间获取周一开始日期，到周日结束日期
	 *
	 * @param current 计算时间
	 * @return 返回开始日期和结束日期数组
	 */
	public static LocalDateTime[] getWeekStartAndEnd(LocalDate current) {
		if (current == null) {
			current = LocalDate.now();
		}
		DayOfWeek week = current.getDayOfWeek();
		int value = week.getValue();
		LocalDate startDate = current.minusDays(value - 1);
		LocalDate endDate = current.plusDays(7 - value);
	 ...
	 * @return java.math.BigDecimal
	 * @author liuzhixian@369zhy.com
	 * @since 2023/4/4 上午10:50
	 */
	public static BigDecimal betweenDays(String startDateStr, String endDateStr) {
		Date startDate = parse(startDateStr, DateUtil.PATTERN_DATETIME);
		Date endDate = parse(endDateStr, DateUtil.PATTERN_DATETIME);
		Duration duration = between(startDate, endDate);
		long seconds = duration.getSeconds();
		return BigDecimal.valueOf(seconds).divide(BigDecimal.valueOf(60 * 60 * 24), 2, BigDecimal.ROUND_UP);
	}
